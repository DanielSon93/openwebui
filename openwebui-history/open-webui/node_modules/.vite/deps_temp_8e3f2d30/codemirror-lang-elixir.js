...
import {
  ...
  syntaxTree,
  tags
} from "./chunk-IWAPA4RT.js";
import {
  keymap
} from "./chunk-OGYNNP47.js";
...
// node_modules/lezer-elixir/dist/index.js
var notIn = 189;
var doubleSlash = 190;
var docAt = 191;
var beforeUnaryOp = 192;
var newlineTerminator = 193;
var newlineBeforeBinaryOperator = 194;
var newlineBeforeDo = 195;
var immediateParenOpen = 1;
var immediateSquareOpen = 2;
var immediateColonWhitespace = 196;
var atomWord = 197;
var keywordWord = 198;
var quotedContentIDouble = 3;
var quotedContentISingle = 4;
var quotedContentIHeredocDouble = 5;
var quotedContentIHeredocSingle = 6;
var quotedContentIParenthesis = 7;
var quotedContentICurly = 8;
var quotedContentISquare = 9;
var quotedContentIAngle = 10;
var quotedContentIBar = 11;
var quotedContentISlash = 12;
var quotedContentDouble = 13;
var quotedContentSingle = 14;
var quotedContentHeredocDouble = 15;
var quotedContentHeredocSingle = 16;
var quotedContentParenthesis = 17;
var quotedContentCurly = 18;
var quotedContentSquare = 19;
var quotedContentAngle = 20;
var quotedContentBar = 21;
var quotedContentSlash = 22;
var SigilModifiers = 23;
var identifier = 199;
var underscoredIdentifier = 200;
var CP_TAB = 9;
var CP_NEWLINE = 10;
var CP_CR = 13;
var CP_SPACE = 32;
var CP_EXCLAMATION_MARK = 33;
var CP_DOUBLE_QUOTE = 34;
var CP_HASH = 35;
var CP_PERCENT = 37;
var CP_AMPERSAND = 38;
var CP_SINGLE_QUOTE = 39;
var CP_PAREN_OPEN = 40;
var CP_PAREN_CLOSE = 41;
var CP_STAR = 42;
var CP_PLUS = 43;
var CP_COMMA = 44;
var CP_MINUS = 45;
...
var CP_COLON = 58;
var CP_SEMICOLON = 59;
var CP_ANGLE_OPEN = 60;
var CP_EQUALS = 61;
var CP_ANGLE_CLOSE = 62;
var CP_QUESTION_MARK = 63;
var CP_AT = 64;
var CP_A_UPPER = 65;
var CP_Z_UPPER = 90;
var CP_SQUARE_OPEN = 91;
var CP_BACKSLASH = 92;
var CP_SQUARE_CLOSE = 93;
var CP_CARET = 94;
var CP_A_LOWER = 97;
var CP_C_LOWER = 99;
var CP_D_LOWER = 100;
...
var CP_Y_LOWER = 121;
var CP_Z_LOWER = 122;
var CP_CURLY_OPEN = 123;
var CP_BAR = 124;
var CP_CURLY_CLOSE = 125;
var CP_TILDE = 126;
var QUOTED_CONTENT_INFOS = [
  {
...
var quotedContentTokens = new ExternalTokenizer((input, stack) => {
  const info = QUOTED_CONTENT_INFOS.find((info2) => stack.canShift(info2.token));
...
function checkQuotedContent(input, info) {
  ...
  for (let hasContent = false; true; hasContent = true) {
    ...
    if (input.next === info.endDelimiter) {
      let length = 1;
      while (length < info.delimiterLength && input.peek(length) === info.endDelimiter) {
        length++;
      }
      ...
      if (input.next === CP_HASH) {
        ...
        else if (info.supportsInterpol || input.peek(1) === info.endDelimiter) {
          return hasContent;
        }
...
var sigilTokens = new ExternalTokenizer((input, stack) => {
  let length = 0;
  while (CP_A_LOWER <= input.next && input.next <= CP_Z_LOWER || CP_A_UPPER <= input.next && input.next <= CP_Z_UPPER || CP_ZERO <= input.next && input.next <= CP_NINE) {
    input.advance();
    length++;
  }
  if (length > 0) {
...
var immediateTokens = new ExternalTokenizer((input, stack) => {
  ...
  } else if (stack.canShift(immediateSquareOpen) && input.next === CP_SQUARE_OPEN) {
    input.advance();
    input.acceptToken(immediateSquareOpen);
  } else if (stack.canShift(immediateColonWhitespace) && input.next === CP_COLON) {
    input.advance();
...
var beforeUnaryOpToken = new ExternalTokenizer((input, stack) => {
  ...
  if (input.next === CP_PLUS) {
    ...
    if (next !== CP_PLUS && next !== CP_COLON && next !== CP_SLASH && !isWhitespace(next)) {
      input.acceptToken(beforeUnaryOp);
    }
  } else if (input.next === CP_MINUS) {
    const next = input.peek(1);
    if (next !== CP_MINUS && next !== CP_ANGLE_CLOSE && next !== CP_COLON && next !== CP_SLASH && !isWhitespace(next)) {
      input.acceptToken(beforeUnaryOp);
    }
  }
});
var ATOM_START = /[\p{ID_Start}_]/u;
var ATOM_CONTINUE = /[\p{ID_Continue}@]/u;
var atomWordToken = new ExternalTokenizer((input, stack) => {
...
var keywordWordToken = new ExternalTokenizer((input, stack) => {
  let [nextCodepointString, nextOffset] = peekCodepoint(input);
  if (ATOM_START.test(nextCodepointString)) {
    do {
      input.advance(nextOffset);
...
var IDENTIFIER_START = /[_\p{Ll}\p{Lm}\p{Lo}\p{Nl}\u1885\u1886\u2118\u212E\u309B\u309C]/u;
var IDENTIFIER_CONTINUE = /[\p{ID_Continue}]/u;
var identifierToken = new ExternalTokenizer((input, stack) => {
  let [nextCodepointString, nextOffset] = peekCodepoint(input);
...
var notInToken = new ExternalTokenizer((input, stack) => {
...
var doubleSlashToken = new ExternalTokenizer((input, stack) => {
...
var atToken = new ExternalTokenizer((input, stack) => {
  if (input.next === CP_AT) {
    input.advance();
    if (input.next === CP_D_LOWER && input.peek(1) === CP_O_LOWER && input.peek(2) === CP_C_LOWER && isWhitespace(input.peek(3)) || input.next === CP_M_LOWER && input.peek(1) === CP_O_LOWER && input.peek(2) === CP_D_LOWER && input.peek(3) === CP_U_LOWER && input.peek(4) === CP_L_LOWER && input.peek(5) === CP_E_LOWER && input.peek(6) === CP_D_LOWER && input.peek(7) === CP_O_LOWER && input.peek(8) === CP_C_LOWER && isWhitespace(input.peek(9)) || input.next === CP_T_LOWER && input.peek(1) === CP_Y_LOWER && input.peek(2) === CP_P_LOWER && input.peek(3) === CP_E_LOWER && input.peek(4) === CP_D_LOWER && input.peek(5) === CP_O_LOWER && input.peek(6) === CP_C_LOWER && isWhitespace(input.peek(7))) {
      input.acceptToken(docAt);
    }
  }
});
var newlineTokens = new ExternalTokenizer((input, stack) => {
  if (input.next === CP_CR) {
    input.advance();
  }
  if (input.next === CP_NEWLINE) {
    ...
    if (stack.canShift(newlineBeforeBinaryOperator) && isOperator(input, offset)) {
      input.acceptToken(newlineBeforeBinaryOperator);
      return;
    }
    if (stack.canShift(newlineTerminator) && input.peek(offset) !== CP_SEMICOLON && input.peek(offset) !== CP_HASH && !newlineAhead) {
...
function isOperator(input, offset = 0) {
  if (input.peek(offset) === CP_AMPERSAND) {
    ...
    offset++;
    if (input.peek(offset) === CP_EQUALS || input.peek(offset) === CP_MINUS || input.peek(offset) === CP_ANGLE_CLOSE) {
      offset++;
      return isOperatorEnd(input, offset);
    } else if (input.peek(offset) === CP_TILDE) {
      offset++;
      ...
      offset++;
      if (input.peek(offset) === CP_ANGLE_OPEN || input.peek(offset) === CP_TILDE) {
        offset++;
        return isOperatorEnd(input, offset);
      }
    } else {
...
var TOKEN_TERMINATORS = /* @__PURE__ */ new Set([
  // Operator starts
  CP_AT,
  CP_DOT,
  CP_PLUS,
...
function peekCodepoint(input) {
  ...
  if (55296 <= next && next <= 56319) {
    const nextNext = input.peek(1);
    if (56320 <= nextNext && nextNext <= 57343) {
      return [String.fromCharCode(next, nextNext), 2];
    }
  }
  return [String.fromCharCode(next), 1];
}
var elixirHighlighting = styleTags({
  "Atom QuotedAtom QuotedAtom/QuotedContent Keyword QuotedKeyword QuotedKeyword/QuotedContent": tags.atom,
  ...
  SpecialIdentifier: tags.special(tags.variableName),
  UnderscoredIdentifier: tags.comment,
  "String String/QuotedContent": tags.string,
  "Charlist Charlist/QuotedContent": tags.string,
  "StringSigil StringSigil/SigilName StringSigil/SigilModifiers StringSigil/QuotedContent StringSigil/{ StringSigil/} StringSigil/[ StringSigil/] StringSigil/( StringSigil/)": tags.string,
  "Sigil Sigil/SigilName Sigil/SigilModifiers Sigil/QuotedContent Sigil/{ Sigil/} Sigil/[ Sigil/] Sigil/( Sigil/)": tags.special(tags.string),
  EscapeSequence: tags.escape,
  "Interpolation/#{ Interpolation/}": tags.special(tags.brace),
  "( )": tags.paren,
  "[ ]": tags.squareBracket,
  "% { }": tags.brace,
  ", ;": tags.separator,
  "<< >>": tags.angleBracket,
  // Explicit keywords
  "fn do end catch rescue after else": tags.keyword,
  // Operators
  Operator: tags.operator,
  WordOperator: tags.keyword,
  "CaptureOperand/Integer": tags.operator,
  // Module attribute
  "AtOperator/Operator AtOperator/Identifier AtOperator/Boolean AtOperator/Nil AtOperator/Call/Identifier": tags.attributeName,
  // Doc string (we don't use ! so that interpolation is not overridden)
  "DocAtOperator/Operator DocAtOperator/Identifier DocAtOperator/Call/Identifier DocAtOperator/Call/Arguments/Boolean DocAtOperator/Call/Arguments/String DocAtOperator/Call/Arguments/String/QuotedContent DocAtOperator/Call/Arguments/Charlist DocAtOperator/Call/Arguments/Charlist/QuotedContent DocAtOperator/Call/Arguments/StringSigil DocAtOperator/Call/Arguments/StringSigil/QuotedContent DocAtOperator/Call/Arguments/StringSigil/SigilName DocAtOperator/Call/Arguments/StringSigil/SigilModifiers DocAtOperator/Call/Arguments/StringSigil/{ DocAtOperator/Call/Arguments/StringSigil/} DocAtOperator/Call/Arguments/StringSigil/[ DocAtOperator/Call/Arguments/StringSigil/] DocAtOperator/Call/Arguments/StringSigil/( DocAtOperator/Call/Arguments/StringSigil/)": tags.docString,
  // Function calls
  "Call/Identifier Call/UnderscoredIdentifier": tags.function(tags.variableName),
  "Call/Dot/Right/Identifier Call/Dot/Right/SpecialIdentifier Call/Dot/Right/UnderscoredIdentifier": tags.function(tags.variableName),
  "Call/Dot/Atom": tags.namespace,
  // Pipe into identifier
  "PipeOperator/Right/Identifier": tags.function(tags.variableName),
  // Function definitions
  "FunctionDefinitionCall/Identifier": tags.keyword,
  "FunctionDefinitionCall/Arguments/Identifier FunctionDefinitionCall/Arguments/UnderscoredIdentifier FunctionDefinitionCall/Arguments/SpecialIdentifier": tags.definition(tags.function(tags.variableName)),
  "FunctionDefinitionCall/Arguments/Call/Identifier FunctionDefinitionCall/Arguments/Call/UnderscoredIdentifier FunctionDefinitionCall/Arguments/Call/SpecialIdentifier": tags.definition(tags.function(tags.variableName)),
  "FunctionDefinitionCall/Arguments/WhenOperator/Identifier FunctionDefinitionCall/Arguments/WhenOperator/UnderscoredIdentifier FunctionDefinitionCall/Arguments/WhenOperator/SpecialIdentifier": tags.definition(tags.function(tags.variableName)),
  "FunctionDefinitionCall/Arguments/WhenOperator/Call/Identifier FunctionDefinitionCall/Arguments/WhenOperator/Call/UnderscoredIdentifier FunctionDefinitionCall/Arguments/WhenOperator/Call/SpecialIdentifier": tags.definition(tags.function(tags.variableName)),
  "FunctionDefinitionCall/Arguments/PipeOperator/Right/Identifier": tags.variableName,
  // Kernel built-in
  "KernelCall/Identifier": tags.keyword
});
var spec_identifier = { __proto__: null, when: 432, def: 446, defdelegate: 448, defguard: 450, defguardp: 452, defmacro: 454, defmacrop: 456, defn: 458, defnp: 460, defp: 462, defexception: 464, defimpl: 466, defmodule: 468, defoverridable: 470, defprotocol: 472, defstruct: 474, alias: 476, case: 478, cond: 480, for: 482, if: 484, import: 486, quote: 488, raise: 490, receive: 492, require: 494, reraise: 496, super: 498, throw: 500, try: 502, unless: 504, unquote: 506, unquote_splicing: 508, use: 510, with: 512, true: 514, false: 516, nil: 518, not: 588, do: 600, after: 604, rescue: 608, catch: 612, else: 616, end: 618, and: 624, in: 626, or: 628, fn: 630 };
var spec_underscoredIdentifier = { __proto__: null, __MODULE__: 436, __DIR__: 438, __ENV__: 440, __CALLER__: 442, __STACKTRACE__: 444 };
var parser = LRParser.deserialize({
  ...
  propSources: [elixirHighlighting],
  skippedNodes: [0, 24],
  repeatNodeCount: 29,
  ...
  tokenizers: [notInToken, doubleSlashToken, atToken, beforeUnaryOpToken, newlineTokens, immediateTokens, atomWordToken, keywordWordToken, quotedContentTokens, sigilTokens, identifierToken, 0, 1, 2],
  topRules: { "Source": [0, 25] },
  dynamicPrecedences: { "43": 1, "79": -1, "150": -1 },
  specialized: [{ term: 199, get: (value) => spec_identifier[value] || -1 }, { term: 200, get: (value) => spec_underscoredIdentifier[value] || -1 }],
  tokenPrec: 42923
});

// node_modules/codemirror-lang-elixir/dist/index.js
var elixirLanguage = LRLanguage.define({
  name: "elixir",
...
function withContinuedStabClause(baseStrategy) {
  return (context) => {
    ...
    if (((_b = (_a = context.node.lastChild) === null || _a === void 0 ? void 0 : _a.type) === null || _b === void 0 ? void 0 : _b.name) === "end" && context.textAfter.endsWith("end")) {
      return context.baseIndentFor(context.node);
    }
    if (((_c = before === null || before === void 0 ? void 0 : before.type) === null || _c === void 0 ? void 0 : _c.name) === "StabClause" && context.textAfter.endsWith("->")) {
...
var insertNewlineCloseEnd = (view) => {
...
var elixirKeymap = [{ key: "Enter", run: insertNewlineCloseEnd }];
function elixir() {
...
//# sourceMappingURL=codemirror-lang-elixir.js.map
