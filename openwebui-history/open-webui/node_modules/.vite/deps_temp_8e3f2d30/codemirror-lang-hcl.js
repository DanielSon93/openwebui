...
// node_modules/codemirror-lang-hcl/dist/index.js
var hclHighlight = styleTags({
...
var QuotedTemplateStart = 1;
var QuotedTemplateEnd = 2;
var TemplateInterpolationStart = 3;
var TemplateInterpolationEnd = 4;
var TemplateDirectiveStart = 5;
var TemplateDirectiveEnd = 6;
var HeredocIdentifier = 7;
var templateLiteralChunk = 93;
var ContextType = {
  QUOTED_TEMPLATE: 1,
...
var Context = class {
  constructor(parent, type, heredoc_identifier) {
    ...
    this.hash = (parent ? parent.hash + parent.hash << 8 : 0) + (type << 4) + (this.heredoc_identifier === "" ? 0 : hashStr(this.heredoc_identifier));
  }
};
var topCtx = new Context(null, "", "");
var trackTemplate = new ContextTracker({
  start: topCtx,
  shift(context, term, stack, input) {
    ...
    if (term === HeredocIdentifier && context.type !== ContextType.HEREDOC_TEMPLATE) {
...
var scanQuotedTemplateStart = new ExternalTokenizer(
  (input, stack) => {
    if (stack.canShift(QuotedTemplateStart) && !(stack.context && stack.context.type === ContextType.QUOTED_TEMPLATE) && String.fromCharCode(input.next) === '"') {
      input.advance();
...
var scanQuotedTemplateEnd = new ExternalTokenizer(
  (input, stack) => {
    if (stack.canShift(QuotedTemplateEnd) && stack.context && stack.context.type === ContextType.QUOTED_TEMPLATE && String.fromCharCode(input.next) === '"') {
      input.advance();
...
var scanTemplateLiteralChunk = new ExternalTokenizer(
  (input, stack) => {
    if (stack.canShift(templateLiteralChunk) && stack.context && stack.context.type === ContextType.QUOTED_TEMPLATE) {
    ...
    if (stack.canShift(templateLiteralChunk) && stack.context && (stack.context.type === ContextType.QUOTED_TEMPLATE || stack.context.type === ContextType.HEREDOC_TEMPLATE) && input.next !== -1) {
      input.advance();
      return input.acceptToken(templateLiteralChunk);
    }
  },
...
var scanTemplateInterpolationStart = new ExternalTokenizer(
  (input, stack) => {
    if (stack.canShift(TemplateInterpolationStart) && stack.canShift(templateLiteralChunk) && !(stack.context && stack.context.type === ContextType.TEMPLATE_INTERPOLATION) && String.fromCharCode(input.next) === "$") {
      input.advance();
...
var scanTemplateInterpolationEnd = new ExternalTokenizer(
  (input, stack) => {
    if (stack.canShift(TemplateInterpolationEnd) && stack.context && stack.context.type === ContextType.TEMPLATE_INTERPOLATION && String.fromCharCode(input.next) === "}") {
      input.advance();
...
var scanTemplateDirectiveStart = new ExternalTokenizer(
  (input, stack) => {
    if (stack.canShift(TemplateDirectiveStart) && stack.canShift(templateLiteralChunk) && !(stack.context && stack.context.type === ContextType.TEMPLATE_DIRECTIVE) && String.fromCharCode(input.next) === "%") {
      input.advance();
      if (String.fromCharCode(input.next) === "{") {
        input.advance();
...
var scanTemplateDirectiveEnd = new ExternalTokenizer(
  (input, stack) => {
    if (stack.canShift(TemplateDirectiveEnd) && stack.context && stack.context.type === ContextType.TEMPLATE_DIRECTIVE && String.fromCharCode(input.next) === "}") {
      input.advance();
...
var scanHeredocIdentifier = new ExternalTokenizer(
  (input, stack) => {
    if (stack.canShift(HeredocIdentifier) && !(stack.context && stack.context.type === ContextType.HEREDOC_TEMPLATE) && input.next !== -1) {
    ...
    if (stack.canShift(HeredocIdentifier) && stack.canShift(templateLiteralChunk) && stack.context && stack.context.type === ContextType.HEREDOC_TEMPLATE && input.next != -1) {
      ...
      for (let i2 = 0; i2 < expected.length; i2++) {
        if (String.fromCharCode(input.next) === expected[i2]) {
          input.advance();
          advance++;
        } else {
          if (advance) {
...
var spec_Identifier = { __proto__: null, true: 39, false: 41, null: 45, for: 64, in: 66, endfor: 70, if: 76, else: 80, endif: 84 };
...
var hclLanguage = LRLanguage.define({
...
//# sourceMappingURL=codemirror-lang-hcl.js.map
